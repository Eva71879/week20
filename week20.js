...
function makeFour() {
	const multiply = (a, b) => {
		if (a === undefined || b === undefined) {
			throw new Error ('Множитель не определен')
		}
		result = a * b;
		return result;
	}
...
function makeFive() {
	let inputData = -1;
	if (inputData < 0) {
		throw new Error ('Отрицательные числа не допускаются');
	}
...
function makeNine() {
	...
	document.getElementById('result9').innerText = 1;
}
...
function makeTen() {
	let email = 'example.com'; //пример валидного email example.email123@example.co.uk
...
function makeEleven() {
	...
	document.getElementById('result11').innerText = 'Телефонный номер корректен';
	console.log('Телефонный номер корректен');
	}
	
	catch (error) {
		console.log (error.message);
		document.getElementById('result11').innerText = error.message;
	}
}
...
function makeTwelve() {
	let age = 12;
		if (age < 18) {
			...
			document.getElementById('result12').innerText = 'Доступ разрешен';
		}
}
...
function makeThirteen() {
	...
	try {
		if (age < 18) {
			...
			document.getElementById('result13').innerText = 'Доступ разрешен';
		}
	}
	catch (error) {
		document.getElementById('result13').innerText = 'Доступ запрещен для лиц младше 18 лет';
	}
}
...
function makeFourteen() {
	...
	try {
		...
		document.getElementById('result14').innerText = `Размер скидки составляет: ${discount}%`;
	}
	}

	catch (error) {
		console.log(error.message);
		document.getElementById('result14').innerText = error.message;
	}
}
...
function makeFifteen(a, func) {
	...
	try {
		...
		resultFifteen.innerText = error.name;
	}
}
